...
for(j in 1:(ncol(main_data.tf.training)-1)){
  ...
  delta_w = log2((Np*C+0.5)/(Nn*A+0.5))
  
  if (!exists("delta_w.data")){
    assign("delta_w.data", delta_w)
  } else {
    delta_w.data = cbind.data.frame(delta_w.data, delta_w)
  }
}
colnames(delta_w.data) = colnames(main_data.tf.training[,-(ncol(main_data.tf.training))])

for(i in 1:nrow(main_data.tf.training)){
  main_data.tf.training[i,-ncol(main_data.tf.training)] = round((main_data.tf.training[i,-ncol(main_data.tf.training)] * delta_w.data),2)
}
#data training - end
...
for(i in 1:nrow(main_data.tf.testing)){
  main_data.tf.testing[i,-ncol(main_data.tf.testing)] = round((main_data.tf.testing[i,-ncol(main_data.tf.testing)] * delta_w.data),2)
}
#data testing - end