...
main_data.positive = main_data.binary[which(main_data.binary$class_label == "positif"), ]
main_data.negative = main_data.binary[which(main_data.binary$class_label == "negatif"), ]
...
for(i in 1:nrow(main_data.tf)){
  for(j in 1:ncol(main_data.tf)-1){
    ...
    #print(paste(rownames(main_data.tf)[i]," ",colnames(main_data.tf)[j]," ",main_data.tf[i,j]," ",A," ",C))
...
corpus = VCorpus(DataframeSource(main_data.testing[,1:2]))
